...
class YahooInfo():
    ...
    def _get_info(self, ticker: str) -> Response:
        ...
        return self.session.get(url=self._make_url(ticker=ticker),
    ...
    def _make_url(self, ticker: str) -> str:
        ''' Make the URL '''
        return f'{self.URL}{ticker}/profile?'
    ...
    def ticker_info(self, ticker: str) -> dict:
        ...
        class_type = 'span'
        soup = BeautifulSoup(
            self._get_info(ticker=ticker.upper())._content.decode('utf-8'),
            features="lxml")
        info = soup.find_all(class_type, class_=class_id)[1:]
        ticker_profile = {'symbol': ticker.upper()}
        if len(info) < 3:
            return ticker_profile
        ticker_profile.update({'industry': info[0].text,
                               'sector': info[1].text,
                               'employees': info[2].text.replace(',', '')})
        return ticker_profile 


# a = YahooInfo()
# print(a.ticker_info('AAPL'))
